...
function M.fetch_container_hostname(container_name)
	local hostname, err = utils.execute_cmd(
		'docker inspect ' .. container_name .. " --format='{{.Config.Hostname}}'",
		{ trim = true }
	)
	if err then
...
function M.fetch_container_gateway(container_name)
	...
	local gateway
	gateway, err = utils.execute_cmd(
		'docker inspect ' .. hostname .. " --format='{{range .NetworkSettings.Networks}}{{.Gateway}}{{end}}'",
		{ trim = true }
	)
	if err then
		return err
...
function M.fetch_container_ip_adderss(container_name)
	...
	local ip_address
	ip_address, err = utils.execute_cmd(
